...
class CustomCursorPagination(CursorPagination):
    ...
    ordering = '-id'  # Order by the 'id' field in descending order
    cursor_query_param = 'cursor'  # Default cursor query parameter

class BookViewSet(ModelViewSet):
    serializer_class = BookSerializer
    permission_classes = [IsAuthenticated]  # Ensure the user is logged in for all actions
    pagination_class = CustomCursorPagination  # Use the custom cursor pagination class
